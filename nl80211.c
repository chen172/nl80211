			      ...
			      int flags, struct nl80211_dump_wiphy_state *state)
{
	...
	//printk("commuincatin get into nl80211_send_wiphy\n");
...
//这个是命令NL80211_CMD_GET_WIPHY的标准的回调函数
static int nl80211_get_wiphy(struct sk_buff *skb, struct genl_info *info)
{
	
	struct sk_buff *msg;
	//从接收到的信息中得到这个字段,通过这个结构体和cfg80211通信
	struct cfg80211_registered_device *rdev = info->user_ptr[0];
	struct nl80211_dump_wiphy_state state = {};

	printk("genl_op's command NL80211_CMD_GET_WIPHY do it: nl80211_get_wiphy\n");
	//分配4096自己大小的套接字buffer
...
//命令NL80211_CMD_NEW_INTERFACE的回调函数
...
//命令NL80211_CMD_DEL_INTERFACE的回调函数
...
//命令NL80211_CMD_SET_NOACK_MAP的回调函数
